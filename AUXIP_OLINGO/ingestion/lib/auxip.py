...
def get_token_info(user,password,mode='dev'):
    ...
    data = {"username":user, "password":password,"client_id":"auxip","grant_type":"password"}

    token_endpoint = "https://dev.reprocessing-preparation.ml/auth/realms/auxip/protocol/openid-connect/token"
    if mode == 'prod':
        token_endpoint = "https://reprocessing-preparation.ml/auth/realms/auxip/protocol/openid-connect/token"
...
def refresh_token_info(token_info,timer):
    ...
    if timer < 900 :
        ...
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        data = {"refresh_token":token_info['refresh_token'],"client_id":"auxip","grant_type":"refresh_token"}
        token_endpoint = "https://reprocessing-preparation.ml/auth/realms/auxip/protocol/openid-connect/token"
        response = requests.post(token_endpoint,data=data,headers=headers)
        return response.json()  
...
def post_to_auxip(access_token,path_to_auxiliary_data_file,uuid,mode='dev'):
    ...
    try:
        ...
        if mode == 'prod':
            auxip_endpoint = "https://reprocessing-preparation.ml/auxip.svc/Products"
