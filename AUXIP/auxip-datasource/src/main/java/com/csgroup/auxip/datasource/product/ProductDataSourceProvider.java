 ...
 */
package com.csgroup.auxip.datasource.product;
...
import com.csgroup.auxip.datasource.product.ProductStrategyBuilder;
 ...
 */
@Component
public class ProductDataSourceProvider implements DataSourceProvider {
    private static final Logger LOG = LoggerFactory.getLogger(ProductDataSourceProvider.class);

    @Autowired
    private ProductDataSource inMemoryDataSource;

    @Override
    public boolean isSuitableFor(ODataRequestContext oDataRequestContext, String entityType) throws ODataDataSourceException {
        return oDataRequestContext.getEntityDataModel().getType(entityType).getJavaType().equals(Product.class);
    }
    ...
    public QueryOperationStrategy getStrategy(ODataRequestContext oDataRequestContext, QueryOperation queryOperation, TargetType targetType) throws ODataException {
        ProductStrategyBuilder builder = new ProductStrategyBuilder();
        List<Predicate<Product>> predicateList = builder.buildCriteria(queryOperation, oDataRequestContext);
        ...
        return () -> {
            ...
            Stream<Product> personStream = inMemoryDataSource.getPersonConcurrentMap().values().stream();

            List<Product> filteredPersons = personStream.filter(p -> predicateList.stream()
