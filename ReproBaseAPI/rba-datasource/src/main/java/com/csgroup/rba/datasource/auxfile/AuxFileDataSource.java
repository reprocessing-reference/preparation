 ...
 */
@Component
public class AuxFileDataSource implements DataSource {
	private static final Logger LOG = LoggerFactory.getLogger(AuxFileDataSource.class);

    private ConcurrentMap<UUID, AuxFile> personConcurrentMap = new ConcurrentHashMap<>();


    @Override
    public Object create(ODataUri oDataUri, Object o, EntityDataModel entityDataModel) throws ODataException {
    	AuxFile product = (AuxFile) o;
        if(personConcurrentMap.putIfAbsent(product.getId(), product) != null) {
    ...
    public Object update(ODataUri oDataUri, Object o, EntityDataModel entityDataModel) throws ODataException {
    	AuxFile product = (AuxFile) o;
        if(personConcurrentMap.containsKey(product.getId())) {
            personConcurrentMap.put(product.getId(), product);
    ...
    public void delete(ODataUri oDataUri, EntityDataModel entityDataModel) throws ODataException {
        ...
        if(entity.isDefined()) {
        	AuxFile person = (AuxFile) entity.get();
    ...
    public ConcurrentMap<UUID, AuxFile> getPersonConcurrentMap() {
