 ...
 */
@Component
public class SensorDataSourceProvider implements DataSourceProvider {
    private static final Logger LOG = LoggerFactory.getLogger(SensorDataSourceProvider.class);

    @Autowired
    private SensorDataSource inMemoryDataSource;

    @Override
    public boolean isSuitableFor(ODataRequestContext oDataRequestContext, String entityType) throws ODataDataSourceException {
        return oDataRequestContext.getEntityDataModel().getType(entityType).getJavaType().equals(Sensor.class);
    }
    ...
    public QueryOperationStrategy getStrategy(ODataRequestContext oDataRequestContext, QueryOperation queryOperation, TargetType targetType) throws ODataException {
        SensorStrategyBuilder builder = new SensorStrategyBuilder();
        List<Predicate<Sensor>> predicateList = builder.buildCriteria(queryOperation, oDataRequestContext);
        ...
        return () -> {
            ...
            Stream<Sensor> personStream = inMemoryDataSource.getPersonConcurrentMap().values().stream();

            List<Sensor> filteredPersons = personStream.filter(p -> predicateList.stream()
