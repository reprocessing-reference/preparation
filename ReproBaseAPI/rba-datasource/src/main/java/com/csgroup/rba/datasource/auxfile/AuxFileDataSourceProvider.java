 ...
 */
@Component
public class AuxFileDataSourceProvider implements DataSourceProvider {
    private static final Logger LOG = LoggerFactory.getLogger(AuxFileDataSourceProvider.class);

    @Autowired
    private AuxFileDataSource inMemoryDataSource;

    @Override
    public boolean isSuitableFor(ODataRequestContext oDataRequestContext, String entityType) throws ODataDataSourceException {
        return oDataRequestContext.getEntityDataModel().getType(entityType).getJavaType().equals(AuxFile.class);
    }
    ...
    public QueryOperationStrategy getStrategy(ODataRequestContext oDataRequestContext, QueryOperation queryOperation, TargetType targetType) throws ODataException {
        AuxFileStrategyBuilder builder = new AuxFileStrategyBuilder();
        List<Predicate<AuxFile>> predicateList = builder.buildCriteria(queryOperation, oDataRequestContext);
        ...
        return () -> {
            ...
            Stream<AuxFile> personStream = inMemoryDataSource.getPersonConcurrentMap().values().stream();

            List<AuxFile> filteredPersons = personStream.filter(p -> predicateList.stream()
