 ...
 */
@Component
public class BaselineDataSourceProvider implements DataSourceProvider {
    private static final Logger LOG = LoggerFactory.getLogger(BaselineDataSourceProvider.class);

    @Autowired
    private BaselineDataSource inMemoryDataSource;

    @Override
    public boolean isSuitableFor(ODataRequestContext oDataRequestContext, String entityType) throws ODataDataSourceException {
        return oDataRequestContext.getEntityDataModel().getType(entityType).getJavaType().equals(Baseline.class);
    }
    ...
    public QueryOperationStrategy getStrategy(ODataRequestContext oDataRequestContext, QueryOperation queryOperation, TargetType targetType) throws ODataException {
        BaselineStrategyBuilder builder = new BaselineStrategyBuilder();
        List<Predicate<Baseline>> predicateList = builder.buildCriteria(queryOperation, oDataRequestContext);
        ...
        return () -> {
            ...
            Stream<Baseline> personStream = inMemoryDataSource.getPersonConcurrentMap().values().stream();

            List<Baseline> filteredPersons = personStream.filter(p -> predicateList.stream()
