	...
	public List<AuxFile> apply(List<AuxFile> files, 
		...
		Collections.sort(initial_result,
			...
			public int compare(AuxFile a, AuxFile b ) {
				Long min_a = Math.min( 
									Math.abs(RuleUtilities.getDiff(t0.minus(dt0), a.ValidityStart)),
									Math.abs(RuleUtilities.getDiff(a.ValidityStop, t1.plus(dt1)))
									   );
				Long min_b = Math.min(
							Math.abs(RuleUtilities.getDiff(t0.minus(dt0), b.ValidityStart)),
							Math.abs(RuleUtilities.getDiff(b.ValidityStop, t1.plus(dt1)))
						);
				return min_a.compareTo(min_b);
			}
		});

		// for(AuxFile ax : initial_result)
		// {
		// 	Long min_a = Math.min( 
		// 							Math.abs(RuleUtilities.getDiff(t0.minus(dt0), ax.ValidityStart)),
		// 							Math.abs(RuleUtilities.getDiff(ax.ValidityStop, t1.plus(dt1)))
		// 							   );

		// 	System.out.println(ax.FullName + " : "+ min_a);
		// }
