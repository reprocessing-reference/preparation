...
def ingest(auxiliary_data_files, nb_sublist, auxip_user, auxip_password, path_to_mc, mode="dev", wasabi_listing_file=None):
    # =======================================================================
    #               READ / OR GENERATE WASABI LISTING
    # =======================================================================
    wasabi_listing = []
    wasabi_listing_dict = {}
    if not wasabi_listing_file:
        # generating the wasabi listing
        print("Getting the already uploaded aux data files listing from wasabi ....")
        print("This may take a while ....")
        wasabi_listing = generate_wasabi_listing(path_to_mc)
    else:
        with open(wasabi_listing_file) as fid:
            lines = fid.readlines()
            for line in lines:
                # egnore .txt files
                if '.txt' not in line:
    ...
    if nb_sublist == 1:
        if len(not_yet_uploaded) > 0:
            upload_and_post(1, path_to_mc, token_info, not_yet_uploaded, mode)
    else:
        nthread = int(nb_sublist)
        ...
        if len(not_yet_uploaded) > 0:
            for i in range(nthread):
                ...
                thread = myThread(i + 1, path_to_mc, token_info, listing, mode)
                thread.start()
        else:
...
if __name__ == "__main__":
    ...
    ingest(auxiliary_data_files, args.n_sublist, args.user, args.password, args.path_to_mc, args.mode, args.wasabi)
